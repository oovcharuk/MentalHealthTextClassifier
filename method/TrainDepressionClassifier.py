...
# Load dataset
data = pd.read_csv('datasets/multi_disorders/Depression.csv')
...
def format_labels(examples):
    examples['label'] = [1 if label == 'Depression' else 0 for label in examples['label']]
    return examples
...
training_args = TrainingArguments(
    output_dir='./trained_models/depression/results',
    eval_strategy="epoch",
    save_strategy="epoch",
...
plt.title('ROC Curve for Depression Disorder Classification')
...
# Save the best model
model.save_pretrained('./trained_models/depression/best_trained_model')
tokenizer.save_pretrained('./trained_models/depression/best_trained_model')
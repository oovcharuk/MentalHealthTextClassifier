...
from sklearn.metrics import accuracy_score, precision_recall_fscore_support, roc_auc_score, log_loss, confusion_matrix, classification_report
...
# Load dataset
data = pd.read_csv('datasets/multi_disorders/AnxietyDisorder.csv')
...
def format_labels(examples):
    examples['label'] = [1 if label == 'Anxiety Disorder' else 0 for label in examples['label']]
    return examples
...
config = DistilBertConfig.from_pretrained(
    ...
    num_labels=2,
    hidden_dropout_prob=0.2,  # Increased Dropout for hidden layers
    attention_probs_dropout_prob=0.2  # Increased Dropout for attention
)
...
training_args = TrainingArguments(
    output_dir='./trained_models/anxiety/results',
    eval_strategy="epoch",  # Evaluate at the end of each epoch
    save_strategy="epoch",  # Save at the end of each epoch
    per_device_train_batch_size=4,
    per_device_eval_batch_size=4,
    num_train_epochs=8,  # 5 epochs as specified
    weight_decay=0.1,    # Increased for regularization
    logging_dir='./logs',
    logging_steps=5,     # Log every 5 steps for console
...
plt.title('ROC Curve for Anxiety Disorder Classification')
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Non-Anxiety', 'Anxiety'], yticklabels=['Non-Anxiety', 'Anxiety'])
...
# Save the best model
model.save_pretrained('./trained_models/anxiety/best_trained_model')
tokenizer.save_pretrained('./trained_models/anxiety/best_trained_model')
...
# Load dataset
data = pd.read_csv('datasets/multi_disorders/AngerIntermittentExplosiveDisorder.csv')
...
def format_labels(examples):
    examples['label'] = [1 if label == 'Anger/ Intermittent Explosive Disorder' else 0 for label in examples['label']]
    return examples
...
training_args = TrainingArguments(
    output_dir='./trained_models/anger/results',
    eval_strategy="epoch",
    save_strategy="epoch",
...
plt.title('ROC Curve for Anger/Intermittent Explosive Disorder Classification')
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Non-Anger', 'Anger/ Intermittent Explosive Disorder'], yticklabels=['Non-Anger', 'Anger/ Intermittent Explosive Disorder'])
...
# Save the best model
model.save_pretrained('./trained_models/anger/best_trained_model')
tokenizer.save_pretrained('./trained_models/anger/best_trained_model')
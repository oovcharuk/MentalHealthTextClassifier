...
# Load dataset
data = pd.read_csv('datasets/multi_disorders/PanicDisorder.csv')   
...
def format_labels(examples):
    examples['label'] = [1 if label == 'Panic Disorder' else 0 for label in examples['label']]
    return examples
...
training_args = TrainingArguments(
    output_dir='./trained_models/panic/results',
    eval_strategy="epoch",
    save_strategy="epoch",
...
plt.title('ROC Curve for Panic Disorder Classification')
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Non-Panic', 'Panic'], yticklabels=['Non-Panic', 'Panic'])
...
# Save the best model
model.save_pretrained('./trained_models/panic/best_trained_model')
tokenizer.save_pretrained('./trained_models/panic/best_trained_model')